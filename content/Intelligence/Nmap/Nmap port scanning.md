...
>
> 8. The target host sends a TCP `RST` packet to the zombie
> 9. The zombie ignores the unsolicited `RST`; it's **IP ID value is left unchanged**.
> 10. The attacker sends a `SYN`/`ACK` TCP packet to the zombie again to probe for its IPv4 ID.
> 11. The zombie responds with `RST`; its IP ID is incremented by `1`.
