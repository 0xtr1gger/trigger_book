...
- The most common uses of SSH are **remote login** and **command execution**, but it can also be used for file transfers and port forwarding. 
- SSH operates on a client-server model with clients connecting to servers.

>[!important] SSH server listens on TCP port `22` by default.

>[!note]- Common non-standard ports
>SSH is often moved to non-standard ports. The top most common non-standard SSH ports (see [`Shodan SSH Facet Analysis`](https://www.shodan.io/search/facet?query=ssh&facet=port)):
>- `2222`
>- `24442`
>- `100001`
>- `1080`
...
>- `222`
>
>This provides no real security, but defeats casual scanners.

- SSH operates as a layered protocol suite, and consists of three main components:
	- **Transport layer** — handles server authentication, key exchange, encryption, and integrity.
	- **User authentication layer** — authenticates the user to the server using one of the supported methods, such as public-key or password authentication.
	- **Connection protocol** — multiplexes a single encrypted tunnel into multiple independent channels, such as PTY, command execution, port forwarding, and file transfer.

>[!note] For more on how SSH works, see [[🛠️ How SSH works]].
## Connecting over SSH
...
>[!example]-
> ```bash
> sudo nmap -Pn -sV -p 22 10.129.20.11 --script ssh-auth-methods,ssh2-enum-algos,sshv1
> ```
> 
> ```bash
> Starting Nmap 7.94SVN ( https://nmap.org ) at 2026-04-13 10:29 CDT
> Nmap scan report for silentium.htb (10.129.20.11)
...
>[`ssh-audit`](https://github.com/jtesta/ssh-audit) is an open-source tool for automated SSH server and client configuration auditing.

>[!note] `ssh-audit` works both on Linux and Windows, supports Python `3.9`-`3.13`, and has no dependencies. 
...
- Fetch platform specific hardening guide:
...
```bash
ssh-audit --get-hardening-guide ubuntu_20_04
```
...
- **Information leak**
	- Even before authentication, during connection negotiation, SSH can reveal:
...
- **Username enumeration**
	- Certain OpenSSH versions (< `7.7`) are vulnerable to username enumeration through timing differences in error responses. The server takes slightly longer to process a valid username (even if the credential is wrong) because it goes further through the authentication flow.

>[!example]-
> ```bash
> msf6 > use auxiliary/scanner/ssh/ssh_enumusers
> msf6 auxiliary(scanner/ssh/ssh_enumusers) > set RHOSTS 10.10.10.50
> msf6 auxiliary(scanner/ssh/ssh_enumusers) > set USER_FILE /usr/share/seclists/Usernames/top-usernames-shortlist.txt
...
- Password spraying with [NetExec](https://www.netexec.wiki/ssh-protocol/password-spraying):

```bash
nxc ssh <targets>/<network_prefix> -u usernames.txt -p /usr/share/seclists/Passwords/Leaked-Databases/rockyou.txt --no-bruteforce
```
...
- Write your private SSH key to the `~/.ssh/authorizied_keys` in the home directory of the target user:

```bash
echo 'AAAB...' >> ~/.ssh/autohrized_keys
```
...
```bash
scp <username>@<target>:/path/to/remote/file /path/to/local/destination
```

- Upload a file to an SSH server:

```bash
scp /path/to/local/file <username>@<target>:/path/to/remote/destination
```
...
- [`SSH Penetration Testing (Port 22) — Hacking Articles`](https://www.hackingarticles.in/ssh-penetration-testing-port-22/)
- [`22 - Pentesintg SSH/SFTP — HackTricks`](https://hacktricks.wiki/en/network-services-pentesting/pentesting-ssh.html#22---pentesting-sshsftp)