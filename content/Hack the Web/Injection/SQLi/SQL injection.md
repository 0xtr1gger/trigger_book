...
>[!note] See [[SQL injection cheat sheet]].
...
>[!note] See [[SQL injection cheat sheet#UNION SELECT]].
...
>[!note] See [[SQL injection cheat sheet#ORDER BY]].
...
- `UNION`-select `NULL` columns and increment the number of selected columns until the query succeeds:
```SQL
...
>[!note] See [[SQL injection cheat sheet#String concatenation]].
