...
> **Required privilege:** Local Administrator or `SYSTEM` (requires `SeDebugPrivilege`).  
> **Goal:** Extract secrets stored by the Windows LSA, such as cached domain credentials, service account passwords, machine account credentials, and other authentication material.
...
>The **`SECURITY` hive** stores Windows **Local Security Authority data** and **LSA secrets**.

>[!note] To learn more about the LSA, see [[LSASS memory#Understanding LSA and LSASS]].
...
>[!important] LSA secrets include **service and system account credentials**, **RAS and SQL connection passwords**, **[EFS](https://en.wikipedia.org/wiki/Encrypting_File_System) encryption keys**, **Internet Explorer passwords**, **RDP keys**, **NT/LM hashes**, **Domain Cached Credentials** (DCC1 and DCC2 hashes), and more.
